...
local gears = {
  ...
  {"Pretty Pretty Princess Scepter", 114020480},
...
local old = playerGui:FindFirstChild("ZenGui")
if old then old:Destroy() end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "ZenGui"
...
-- Custom vector-style icons drawn from plain Frames (no emoji glyphs, so they
-- render identically everywhere instead of depending on font/emoji support).
local function drawIcon(parent, kind, color, size)
    size = size or 16
    ...
    local function bar(w, h, rot, cx, cy)
    ...
    if kind == "close" or kind == "clear" then
        bar(size*0.85, 2, 45, 0.5, 0.5)
        bar(size*0.85, 2, -45, 0.5, 0.5)
    elseif kind == "search" then
        local ring = Instance.new("Frame", holder)
        ring.Size = UDim2.new(0, size*0.6, 0, size*0.6)
        ...
        bar(size*0.48, 2, 45, 0.8, 0.8)
    elseif kind == "copy" then
        local back = Instance.new("Frame", holder)
        back.Size = UDim2.new(0, size*0.58, 0, size*0.58)
        ...
        backStroke.Color = color
        backStroke.Thickness = 1.5
        local front = Instance.new("Frame", holder)
        front.Size = UDim2.new(0, size*0.58, 0, size*0.58)
        front.Position = UDim2.new(0, size*0.34, 0, size*0.34)
        front.BackgroundColor3 = color
        front.BorderSizePixel = 0
        Instance.new("UICorner", front).CornerRadius = UDim.new(0, 3)
    elseif kind == "tag" then
        local dia = Instance.new("Frame", holder)
        dia.Size = UDim2.new(0, size*0.6, 0, size*0.6)
        dia.AnchorPoint = Vector2.new(0.5, 0.5)
        dia.Position = UDim2.new(0.5, 0, 0.5, 0)
        dia.Rotation = 45
        dia.BackgroundTransparency = 1
        dia.BorderSizePixel = 0
        Instance.new("UICorner", dia).CornerRadius = UDim.new(0, 3)
        local diaStroke = Instance.new("UIStroke", dia)
        diaStroke.Color = color
        diaStroke.Thickness = 1.5
        local dot = Instance.new("Frame", holder)
        dot.Size = UDim2.new(0, 3, 0, 3)
        dot.AnchorPoint = Vector2.new(0.5, 0.5)
        dot.Position = UDim2.new(0.3, 0, 0.5, 0)
        dot.BackgroundColor3 = color
        dot.BorderSizePixel = 0
        Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    elseif kind == "send" then
        bar(size*0.55, 2, 35, 0.32, 0.3)
        bar(size*0.55, 2, -35, 0.32, 0.7)
    elseif kind == "logo" then
        local outer = Instance.new("Frame", holder)
        ...
        inner.Size = UDim2.new(0, size*0.32, 0, size*0.32)
...
-- Toggle Button (white glass pill, icon + label, drop shadow, press feedback)
local ToggleBtn = Instance.new("TextButton")
ToggleBtn.Name = "ToggleBtn"
ToggleBtn.Size = UDim2.new(0, 118, 0, 48)
ToggleBtn.Position = UDim2.new(0, 14, 0.5, -24)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
ToggleBtn.BackgroundTransparency = 0.28
...
-- Subtle glossy highlight over the white glass
...
ToggleStroke.Color = Color3.fromRGB(70, 130, 220)
...
ToggleIconBadge.ClipsDescendants = true
ToggleIconBadge.ZIndex = 6
Instance.new("UICorner", ToggleIconBadge).CornerRadius = UDim.new(1, 0)

-- Logo image (fetched once and cached locally as a custom asset).
-- Falls back to a text icon if the executor doesn't support HttpGet/writefile/getcustomasset.
local ToggleLogo = Instance.new("ImageLabel", ToggleIconBadge)
ToggleLogo.Size = UDim2.new(1, 0, 1, 0)
...
local ToggleIconFallback = drawIcon(ToggleIconBadge, "logo", Color3.fromRGB(50, 60, 90), 20)
ToggleIconFallback.ZIndex = 7

local LOGO_URL = "https://raw.githubusercontent.com/ZeNZxyn/Zen-HtmlXD/refs/heads/main/ZenCover.jpg"
task.spawn(function()
    local ok = pcall(function()
        local httpGet = (syn and syn.request) or http_request or request
        local raw
        if httpGet then
            local res = httpGet({Url = LOGO_URL, Method = "GET"})
            raw = res and res.Body
        elseif game.HttpGetAsync then
            raw = game:HttpGetAsync(LOGO_URL)
        end
        if raw and writefile and getcustomasset then
            writefile("ZenGuiLogo.jpg", raw)
            ToggleLogo.Image = getcustomasset("ZenGuiLogo.jpg")
        elseif raw and getsynasset then
            writefile("ZenGuiLogo.jpg", raw)
            ToggleLogo.Image = getsynasset("ZenGuiLogo.jpg")
        else
            error("no compatible http/file functions available")
        end
    end)
    ToggleIconFallback.Visible = not ok
end)
...
ToggleLabel.Text = "ZenGui"
ToggleLabel.TextColor3 = Color3.fromRGB(45, 55, 85)
...
local open = false
local CLOSED_COLOR, CLOSED_COLOR_DOWN = Color3.fromRGB(70, 130, 220), Color3.fromRGB(50, 105, 190)
local OPEN_COLOR, OPEN_COLOR_DOWN = Color3.fromRGB(210, 70, 70), Color3.fromRGB(180, 50, 50)

ToggleBtn.MouseButton1Down:Connect(function()
    ToggleBtn.BackgroundTransparency = 0.1
    ToggleStroke.Color = open and OPEN_COLOR_DOWN or CLOSED_COLOR_DOWN
end)
ToggleBtn.MouseButton1Up:Connect(function()
    ToggleBtn.BackgroundTransparency = 0.28
    ToggleStroke.Color = open and OPEN_COLOR or CLOSED_COLOR
end)
...
MainFrame.BackgroundColor3 = Color3.fromRGB(250, 250, 253)
...
MainStroke.Color = Color3.fromRGB(210,215,230)
...
tbGradient.Color = ColorSequence.new(Color3.fromRGB(255,255,255), Color3.fromRGB(240,241,247))
...
drawIcon(TitleIcon, "logo", Color3.fromRGB(70,120,220), 16)

local TitleLabel = Instance.new("TextLabel", TitleBar)
TitleLabel.Size = UDim2.new(1,-110,1,0)
TitleLabel.Position = UDim2.new(0,40,0,0)
TitleLabel.BackgroundTransparency = 1
TitleLabel.Text = "ZenGui"
TitleLabel.TextColor3 = Color3.fromRGB(30, 32, 45)
...
TitleCount.TextColor3 = Color3.fromRGB(120,125,150)
TitleCount.Font = Enum.Font.BuilderSans
...
CloseBtn.BackgroundColor3 = Color3.fromRGB(235,236,243)
CloseBtn.BackgroundTransparency = 0.2
CloseBtn.AutoButtonColor = true
CloseBtn.BorderSizePixel = 0
CloseBtn.Text = ""
Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(1,0)
drawIcon(CloseBtn, "close", Color3.fromRGB(85,90,115), 12)
CloseBtn.MouseEnter:Connect(function() CloseBtn.BackgroundColor3 = Color3.fromRGB(220,70,70) end)
CloseBtn.MouseLeave:Connect(function() CloseBtn.BackgroundColor3 = Color3.fromRGB(235,236,243) end)
...
SearchFrame.BackgroundColor3 = Color3.fromRGB(240,241,247)
...
SearchIcon.Text = ""
drawIcon(SearchIcon, "search", Color3.fromRGB(120,125,160), 14)
...
ClearBtn.BackgroundColor3 = Color3.fromRGB(220,222,232)
...
drawIcon(ClearBtn, "clear", Color3.fromRGB(80,84,110), 9)
...
SearchBox.PlaceholderColor3 = Color3.fromRGB(140,142,165)
SearchBox.Text = ""
SearchBox.TextColor3 = Color3.fromRGB(30,32,45)
...
NotifyLabel.Font = Enum.Font.BuilderSansMedium
...
ClipHint.Font = Enum.Font.BuilderSansMedium
...
ClipBox.Font = Enum.Font.BuilderSansBold
ClipBox.TextSize = 13
ClipBox.ZIndex = 21
...
for i, gear in ipairs(gears) do
    local ok, err = pcall(function()
        ...
        local baseColor = (i%2==0) and Color3.fromRGB(255,255,255) or Color3.fromRGB(244,245,250)
        ...
        RowStroke.Color = Color3.fromRGB(210,215,230)
        ...
        NameLbl.Size = UDim2.new(1,-176,0,22)
        NameLbl.Position = UDim2.new(0,68,0,9)
        NameLbl.BackgroundTransparency = 1
        NameLbl.Text = name
        NameLbl.TextColor3 = Color3.fromRGB(35,37,52)
        ...
        IDLbl.Size = UDim2.new(1,-176,0,18)
        IDLbl.Position = UDim2.new(0,68,0,33)
        IDLbl.BackgroundTransparency = 1
        IDLbl.Text = "ID " .. id
        IDLbl.TextColor3 = Color3.fromRGB(90,115,190)
        IDLbl.Font = Enum.Font.BuilderSans
        IDLbl.TextSize = 11
        IDLbl.TextXAlignment = Enum.TextXAlignment.Left

        -- Three compact icon buttons: Copy ID, Copy Name, Send
        local CopyBtn = Instance.new("TextButton", Row)
        CopyBtn.Size = UDim2.new(0,32,0,32)
        CopyBtn.Position = UDim2.new(1,-108,0.5,-16)
        CopyBtn.BackgroundColor3 = Color3.fromRGB(235,236,244)
        CopyBtn.AutoButtonColor = true
        CopyBtn.BorderSizePixel = 0
        CopyBtn.Text = ""
        Instance.new("UICorner", CopyBtn).CornerRadius = UDim.new(0,8)
        local CopyStroke = Instance.new("UIStroke", CopyBtn)
        CopyStroke.Color = Color3.fromRGB(210,215,230)
        CopyStroke.Transparency = 0.5
        drawIcon(CopyBtn, "copy", Color3.fromRGB(60,64,90), 14)

        CopyBtn.MouseButton1Click:Connect(function()
            local copied = pcall(function() setclipboard(id) end)
            if copied then
                showNotify("Copied ID: "..id, Color3.fromRGB(50,90,170))
            else
                showClip(id)
            end
        end)
        ...
        -- Copy Name button
        local CopyNameBtn = Instance.new("TextButton", Row)
        CopyNameBtn.Size = UDim2.new(0,32,0,32)
        CopyNameBtn.Position = UDim2.new(1,-72,0.5,-16)
        CopyNameBtn.BackgroundColor3 = Color3.fromRGB(235,236,244)
        CopyNameBtn.AutoButtonColor = true
        CopyNameBtn.BorderSizePixel = 0
        CopyNameBtn.Text = ""
        Instance.new("UICorner", CopyNameBtn).CornerRadius = UDim.new(0,8)
        local CopyNameStroke = Instance.new("UIStroke", CopyNameBtn)
        CopyNameStroke.Color = Color3.fromRGB(210,215,230)
        CopyNameStroke.Transparency = 0.5
        drawIcon(CopyNameBtn, "tag", Color3.fromRGB(60,64,90), 14)

        CopyNameBtn.MouseButton1Click:Connect(function()
            local copied = pcall(function() setclipboard(name) end)
            if copied then
                showNotify("Copied name: "..name, Color3.fromRGB(50,90,170))
            else
                showClip(name)
            end
        end)
        ...
        drawIcon(SendBtn, "send", Color3.new(1,1,1), 14)
        ...
        Row.MouseEnter:Connect(function()
            Row.BackgroundColor3 = Color3.fromRGB(225, 233, 250)
    ...
    if not ok then
        warn("[ZenGui] Row "..i.." failed: "..tostring(err))
    end
end
...
local function setOpen(v)
    open = v
    MainFrame.Visible = v
    ToggleLabel.Text = v and "Close" or "ZenGui"
    ToggleStroke.Color = v and OPEN_COLOR or CLOSED_COLOR
end
...
UserInputService.InputChanged:Connect(function(i)
    ...
    if not toggleMoved and (math.abs(d.X) > TOGGLE_DRAG_THRESHOLD or math.abs(d.Y) > TOGGLE_DRAG_THRESHOLD) then
        toggleMoved = true
    end
    if toggleMoved then
        ...
        ToggleShadow.Position = newPos - UDim2.new(0, 3, 0, -1)
    end
end)
...
print("[ZenGui] Loaded "..#gears.." gears successfully!")
